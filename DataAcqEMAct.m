...
measSet.measTime = 4.2;  % Measurement is x second long. Note that this is inclusive of the set zero padding
...
switch measSet.mode
    case 'sine'
        freqIntrst = 700;   % frequency of interest. Set start and end freqs in an array if mode is 'chirp'
    case 'chirp'
        freqIntrst = [.5 1000];
    case 'square'
        freqIntrst = .5;
...
timeVec = 0:1/fs:(measSet.measTime-2*measSet.zPadLen-1/fs);
zPad = zeros(1,round(measSet.zPadLen*fs));

switch mode
    case 'sine'
        srcSig = swGain*sin(2*pi*timeVec*freqIntrst);
    ...
    case 'chirp'
        srcSig = swGain*chirp(timeVec,freqIntrst(1),timeVec(end),freqIntrst(2),'linear');  % Decide on linear vs logarithmic
        srcSig = [zPad srcSig zPad];    % manually handling zero padding. Not windowing for simplicity
        
    case 'square'
        srcSig = swGain*square(2*pi*timeVec*freqIntrst);  % square fn included inSignal processing toolbox 
        srcSig = [zPad srcSig zPad];    % manually handling zero padding. Not windowing for simplicity
        
    case 'dc_steps'
        amp = linspace(-swGain, swGain, n+Steps);
        for i = 1:nSteps
            indPerStep = round(measTime*fs/nSteps);     % indices per step
    ...
    case 'others'   % other experimental stimuli
        srcSig = swGain*sin(2*pi*175*(0:1/fs:.04)).*hann((fs/25+1))';
        srcSig = swGain*sin(2*pi*175*(0:1/fs:.04)).*exp(-120*(0:1/fs:.04));
        %srcSig = [repmat(zPad,1,3) srcSig repmat(zPad,1,2)];
        
end

% Define all measurement signals anyway
measmnts.velData = zeros(length(srcSig),nReps);
measmnts.currData = zeros(length(srcSig),nReps);
measmnts.forceData.Fx = zeros(length(srcSig),nReps);
measmnts.forceData.Fy = zeros(length(srcSig),nReps);
measmnts.forceData.Fz = zeros(length(srcSig),nReps);
measmnts.thermData = zeros(length(srcSig),nReps);
...
if measSet.force 
    ...
    measSet.MFx=[0.00364 -0.04142 -0.16003 -1.67055 0.09189 1.63189];   % scaling matrix from Mengji's models. I assume these values are directly from the ATI weebsite
...
if measSet.curr         % Current is pretty mandatory, but including it anyway
    currDataFilt = movmean(currData,20);
...
fName = "Data/" + string(mode) + "_" + num2str(currPP,'%.1f') + "_A_pp_"+string(endTag)+".mat";    % choose .mat or .csv
...
if measSet.ldv
    procData.velData = inputDat.(fn{2})*ldvScaling/(4*1000);    % velocities in m/s. Make sure names are accurate
...
if measSet.curr
    procData.currData = inputDat.(fn{3})/.22;  % Current in amps. Measured Voltage across a .22ohm shunt resistor. Ensure correct channel is processed
end
...
if measSet.force
    ...
    for i = 1:length(measSet.forceCh)
        rawMeas(:,i) = inputDat.(fn{i+5});  % Raw measurements from the daq
    end
...
if measSet.therm
    ...
    procData.thermData = inputDat.(fn{12})*scaleFact + ambientT;  % Temperature in degrees celcius
end
